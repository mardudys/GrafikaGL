...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("nep0fds1.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku nep0fds1.tif");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &PLUTO);
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("PLUTO", PLUTO_TEX);
